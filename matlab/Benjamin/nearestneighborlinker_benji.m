function [ target_indices target_distances unassigned_targets ] = nearestneighborlinker_benji(source, target, vitesse_derive)
...
% Jean-Yves Tinevez <jeanyves.tinevez@gmail.com>
    ...
    for i = 1 : n_source_points
        ...
        diff_coords = target - repmat(current_point, n_target_points, 1) - repmat(vitesse_derive, n_target_points, 1);
...
%     while ~all(isinf(D(:)))
        ...
        [ min_D target_indices ] = min(D, [], 2); % index of the closest target for each source points
%         [ var_temp, sorted_index ] = sort(min_D);
