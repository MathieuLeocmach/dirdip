function liste_vertex = trouve_vertex2(image)
...
d = sort([zeros(length(k),1) voisins8]');
nb_voisines = sum(diff(d) ~= 0) + est_pixel_bord';
a_garder = (nb_voisines >= 3);
indices_a_garder = find(nb_voisines >= 3);

liste_vertex = zeros(sum(a_garder),9);
liste_vertex(:,1) = k(a_garder);

liste_vertex(:,2:9) = voisins8(a_garder,:);